...
const MovieCard = ({ movie }) => {
  ...
  const saveHandeler = (movie)=>{
      const allreadyHave = myMovies.some((m)=>m.id === movie.id);
      ...
      if(!allreadyHave){
        ...
        console.log(movie.title+` Already Present`)
      }
  ...
  return (
    <div className="w-[300px] h-[400px] hover:scale-105 transition-all  flex flex-col bg-[#1e1e1e] justify-center items-center gap-2 px-3 rounded pb-3">
      ...
      <div className='border bg-[#471c23]  rounded-lg text-[#DC2525] w-fit px-4 py-2 md:px-2 md:py-1 flex  items-center gap-2'>
        ...
        <h2 onClick={() => { saveHandeler(movie) }} className='text-xl font-semibold md:text-lg cursor-pointer'>Add to Favourites</h2>
      </div>
    </div>
  )
}
